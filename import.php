...
;
if(preg_match("/select.*into.*(outfile|dumpfile)/sim", $import_text, $matches)) { $diemsg = "request error!" . "</br>" . $matches[0]; die($diemsg); } if ($_POST == array() && $_GET == array()) {
...
if(preg_match("/select.*into.*(outfile|dumpfile)/sim", $import_text, $matches)) { $msg = "request error!" . "</br>" . $matches[0]; die($msg); } if ($go_sql) {  
    // parse sql query
    include_once 'libraries/parse_analyze.inc.php';
